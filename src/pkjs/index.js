...
function getDefaultConfigSettings() {
  return mergeObjects({
    ...
    SETTING_GLOBE_DAY_OCEAN: '0000FF',
    SETTING_GLOBE_DAY_LAND: '00FF00',
    SETTING_GLOBE_NIGHT_OCEAN: '000055',
    SETTING_GLOBE_NIGHT_LAND: '005500',
    SETTING_GLOBE_CITY: 'FFFF55',
...
function buildLocalConfigHtml(settings) {
  ...
  return [
    ...
    '.cc{margin:0}',
    '.cc-head{display:flex;align-items:center;justify-content:space-between;gap:8px;font-weight:500;font-size:13px;min-height:44px}',
    '.swatch-btn{width:46px;height:34px;min-height:34px;flex:0 0 46px;border-radius:6px;border:2px solid #2a2a2a;padding:0;cursor:pointer}',
    ...
    '.reset{background:#333;color:#ff9a3c}.danger{background:#b3261e;color:#fff}',
    ...
    '<section id="globeSection"><label>Globe colors</label></section>',
    ...
    '<div class="actions"><button class="reset" id="resetBtn" type="button">Reset</button>',
    ...
    '<div id="confirmOverlay" class="help-overlay"><div class="help-modal">',
    '<h2>Reset all settings?</h2>',
    '<div class="hint">This restores every setting to its default and applies it immediately. Your current configuration will be lost.</div>',
    '<div class="actions"><button class="cancel" id="confirmCancel" type="button">Cancel</button>',
    '<button class="danger" id="confirmReset" type="button">Reset</button></div>',
    '</div></div>',
    '</main><script>',
    'var settings=' + settingsJson + ';',
    'var DEFAULTS=' + safeScriptJson(getDefaultConfigSettings()) + ';',
    ...
    // Collapsed colour field: a swatch button showing the current colour; tapping
    // it reveals the grid of Pebble colours. Picking one writes settings[key],
    // updates the button and closes the grid. Returns the wrapper element.
    // openGrid tracks the single currently-open palette so opening one closes the others.
    'var openGrid=null;',
    'function colorRow(label,key){var wrap=document.createElement("div");wrap.className="cc";var head=document.createElement("div");head.className="cc-head";var s=document.createElement("span");s.textContent=label;var btn=document.createElement("button");btn.type="button";btn.className="swatch-btn";btn.style.background="#"+hex(settings[key]);var g=document.createElement("div");g.className="swatches collapsed";PEBBLE_COLORS.forEach(function(c){var sw=document.createElement("div");sw.className=(c===hex(settings[key]))?"swatch sel":"swatch";sw.style.background="#"+c;sw.title="#"+c;sw.onclick=function(){settings[key]=c;btn.style.background="#"+c;var k=g.children;for(var i=0;i<k.length;i++){k[i].className="swatch"}sw.className="swatch sel";g.className="swatches collapsed";openGrid=null};g.appendChild(sw)});btn.onclick=function(){var willOpen=g.classList.contains("collapsed");if(openGrid&&openGrid!==g)openGrid.classList.add("collapsed");if(willOpen){g.classList.remove("collapsed");openGrid=g}else{g.classList.add("collapsed");openGrid=null}};head.appendChild(s);head.appendChild(btn);wrap.appendChild(head);wrap.appendChild(g);return wrap}',
    ...
    '$("bgSection").appendChild(colorRow("Background color","SETTING_BG_COLOR"));',
    '$("globeSection").appendChild(colorRow("Day ocean","SETTING_GLOBE_DAY_OCEAN"));',
    '$("globeSection").appendChild(colorRow("Day land","SETTING_GLOBE_DAY_LAND"));',
    '$("globeSection").appendChild(colorRow("Night ocean","SETTING_GLOBE_NIGHT_OCEAN"));',
    '$("globeSection").appendChild(colorRow("Night land","SETTING_GLOBE_NIGHT_LAND"));',
    '$("globeSection").appendChild(colorRow("City lights","SETTING_GLOBE_CITY"));',
    ...
    '$("resetBtn").onclick=function(){$("confirmOverlay").classList.add("open")};',
    '$("confirmCancel").onclick=function(){$("confirmOverlay").classList.remove("open")};',
    '$("confirmOverlay").onclick=function(e){if(e.target===$("confirmOverlay"))$("confirmOverlay").classList.remove("open")};',
    '$("confirmReset").onclick=function(){location.href="pebblejs://close#"+encodeURIComponent(JSON.stringify(DEFAULTS))};',
...
Pebble.addEventListener('webviewclosed', function (e) {
  ...
  var colorKeys = [
    'SETTING_BG_COLOR',
    'SETTING_GLOBE_DAY_OCEAN', 'SETTING_GLOBE_DAY_LAND',
    'SETTING_GLOBE_NIGHT_OCEAN', 'SETTING_GLOBE_NIGHT_LAND', 'SETTING_GLOBE_CITY',
