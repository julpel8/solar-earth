...
void draw_ring_layer(Layer *layer, GContext *ctx) {
  ...
  // Base 15h shift (Halcyon) + 12h so the sun sits on the same side as the
  // globe's lit hemisphere: noon at the bottom, midnight at the top.
  int shiftedHour = (hour + 15 + 12) % 24;
  float progress = ((shiftedHour % 24) + (minute / 60.0f)) / 24.0f;

  GPoint sunPos = get_rect_position(progress, sunBounds);

  // Apply the same shift to the sunrise and sunset times
  int shiftedSunriseMinute =
      (currentSolarInfo.sunriseMinute + (15 + 12) * 60) % (24 * 60);
  int shiftedSunsetMinute =
      (currentSolarInfo.sunsetMinute + (15 + 12) * 60) % (24 * 60);
